...
export default function ChatPage() {
  const [room, setRomm] = useState([
    {title: '용산역 일 등 러닝 크루!!',id : 1, max: 4, member:[] }, {id:2, max:3}, {id: 3, max:5}
  ])
  ...
  const join = async () => {
    ...
    // }
    if(currentRoom.member.find()){ //내 유저 아이디와 같은게 있다면 
      //입장
  ...
  return (
    <div>
      ...
      {go ? <button onClick={() => join()}>입장</button> : <Chat setGo={setGo} currentRoom={currentRoom} socket={socket}  />}
    </div>
  )
}
